...
public class AnnotationColor {
  ...
  public static void writeColors() {
    try {
      ...
      for (int i=0; i<colors.size(); i++) {
      	AnnotationColorEntry c = (AnnotationColorEntry) colors.get(i);
  ...
  public static void addType(String type) {
  	for (int i=0; i<colors.size(); i++)
  		if (((AnnotationColorEntry) colors.get(i)).type.equals(type))
  			return;
    String color = "#" + Integer.toHexString(type.hashCode() & 0xffffff);
      ...
      button.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          ...
          if (newColor != null) {
            ...
            ((AnnotationColorEntry) colors.get(buttonNumber)).color =
              "#" + Integer.toHexString(newColor.hashCode());
            writeColors();
          }
        }
      });
...
class AnnotationColorEntry {
    ...
    if (stok.countTokens() == 0) {
      color = "#" + Integer.toHexString(annType.hashCode());
    } else {
