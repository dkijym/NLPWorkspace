...
public class FindAceValues {
	...
	public static void readTypeDict () {
		...
		if (fileName != null) {
			...
			System.err.println ("No Ace.value.fileName specified in config file");
		}
	}
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.charAt(0) == '=') {
					...
					if (type == null) {
						System.err.println ("Error in loading value dictionary.");
						System.err.println ("No type specified for " + tokens[0]);
					} else {
						valueTypeMap.put(tokens[0], type);
					}
				}
			}
			System.err.println ("Value dictionary loaded.");
		} catch (IOException e) {
			System.err.print("Unable to load value dictionary due to exception: ");
	...
	public static void buildAceValues (Document doc, String docId, AceDocument aceDoc) {
		...
		System.out.println ("Built " + aceDoc.values.size() + " values.");
	}
	...
	public static void pruneAceValues (AceDocument aceDoc) {
		...
		System.out.println (aceDoc.values.size() + " values remain after pruning.");
	}
}
