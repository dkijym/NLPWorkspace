...
public class EDTtype {
	...
	static final String ACEdir =
	    home + "ACE 05/V4/";
	static final String typeDictFile = ACEdir + "EDT type dict.txt";
	...
	static int trainingMentions = 0, correct = 0, incorrect = 0, unknown = 0;
 	 ...
 	 *  head, as well as statistics on generics (which were needed until 2004).
 	 */
 	...
 	public static void main (String[] args) throws IOException {
		...
		dataDir = oldACEdir;
		trainFromFileList (oldACEdir + "training all.txt");
		trainFromFileList (oldACEdir + "feb02 all.txt");
		trainFromFileList (oldACEdir + "sep02 all.txt");
		apfFileSuffix = ".apf.xml";
		trainFromFileList (oldACEdir + "aug03 all.txt");
		// train on 2004 data
		AceDocument.ace2004 = true;
		trainFromFileList (oldACEdir + "training04 nwire 21andup.txt");
		trainFromFileList (oldACEdir + "training04 bnews 21andup.txt");
		trainFromFileList (oldACEdir + "training04 chinese.txt");
		// train on 2005 data
		dataDir = ACEdir;
		AceDocument.ace2005 = true;
		trainFromFileList (ACEdir + "tail.txt");
	 ...
	 *  parameter <CODE>Ace.EDTtype.fileName</CODE>.
	 */
	...
	static { // the following nouns are markable if they appear with
	     ...
	     specifiedEDTtype.put("prison", "FACILITY:Building-Grounds");
	     specifiedEDTtype.put("room", "FACILITY:Subarea-Facility");
	     specifiedEDTtype.put("home", "FACILITY:Building-Grounds");
	     specifiedEDTtype.put("state", "GPE:State-or-Province");
	     specifiedEDTtype.put("land", "LOCATION:Region-General");
	     // 'minister' as a noun is markable
	     // 'Minister' as part of a title is not
	     specifiedEDTtype.put("minister", "PERSON:Individual");
	     }
