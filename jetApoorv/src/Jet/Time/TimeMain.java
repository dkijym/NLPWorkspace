...
public class TimeMain {
	...
	private static final Pattern YMDpattern =
		Pattern.compile("(^|\\D)((199|200)\\d{5})($|\\D)");
	...
	public static void processDocument (Document doc) {
		...
		//  tag all TEXT fields
		//
		Vector texts = doc.annotationsOfType("TEXT");
		if (texts == null) {
			return;
		}

		for (int i = 0; i < texts.size(); i++) {
			Annotation text = (Annotation) texts.get(i);
			Span span = text.span();
			if (doc.annotationsOfType("token", span) == null)
				Tokenizer.tokenize(doc, span);
			numberAnnotator.annotate(doc, span);
			System.out.println("Time annotator:" + (timeAnnotator == null));
