...
public class MaxEntNE extends TokenClassifier {
	...
	private Datum NEfeatures (int i, String[] words, Annotation[] tokens, String priorTag,
		...
		//---
		String onomaType = onomaFeature(doc, tokens[i]);
		if (onomaType != null)
			d.addFV ("onoma", onomaType);
		if (Character.isUpperCase(words[i].charAt(0)) && i < words.length - 2) {
			int j = i + 1;
			while (Character.isUpperCase(words[j].charAt(0)) && j < words.length - 2) j = j + 1;
			String onomaType2 = onomaFeature(doc, tokens[j + 1]);
			...
			for (int iState = 0; iState < nStates; iState++) {
				if (iToken == 0) {
					...
					for (int iPrior = 0; iPrior < nStates; iPrior++) {
						...
						double p = Math.log(outcome[iState]);
						if (state[iState].equals("other")) p += otherOffset;
						if (allowedTransition(iPrior, iState) && prob[iToken-1][iPrior] + p > bestProb) {
							bestProb = prob[iToken-1][iPrior] + p;
							bestPrior = iPrior;
						}
					}
					prob[iToken][iState] = bestProb;
					prior[iToken][iState] = bestPrior;
				}
			}
		}
