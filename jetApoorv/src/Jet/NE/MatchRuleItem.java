...
/**
 *  an element of the left-hand (match) portion of an ENE transformation
 *  rule, specifying constraints on a single token.
 *
 *  @author  Akira Oda
 */
...
public class MatchRuleItem {
	...
	public boolean accept(Document doc, Annotation[] tokens, int pos,
		...
		if (partOfSpeechRule != null && !partOfSpeechRule.accept(doc, tokens, pos)) {
			return false;
		}

		if (namedEntityRule != null && !namedEntityRule.accept(doc, tokens, pos)) {
			return false;
		}
