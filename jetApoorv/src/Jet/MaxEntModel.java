...
public class MaxEntModel {
	...
	public void buildModel () {
		...
		try {
			...
			model = GIS.trainModel(es, iterations, cutoff, USE_SMOOTHING);
		} catch (Exception e) {
	...
	public void loadModel (String modelFileName) {
		try {
			...
			model = new SuffixSensitiveGISModelReader(f).getModel();
	...
	public void loadModel (BufferedReader reader) {
		try {
			model = new PlainTextGISModelReader(reader).getModel();
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
