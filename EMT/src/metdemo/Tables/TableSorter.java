...
public class TableSorter extends TableMap {
    ...
    int             m_sortColumn;
    private boolean m_sortingOn;		// turn off message-by-message sorting for batch updates
	...
	if (m_sortingOn || (indexes.length != rowCount))
	{
    ...
    public synchronized void n2sort() {
    ...
    public synchronized void shuttlesort(int from[], int to[], int low, int high) {
        if (high - low < 2) {
            return;
        }
    ...
    public final int translate(int row)
    {
	return indexes[row];
    }
	...
	sortByLastSortColumn();
    }

    public synchronized void setSortingOn(boolean b) {
      m_sortingOn = b;
    }

    public synchronized void sortByLastSortColumn() {
      if (m_sortingOn)
      {
	sortByColumn(m_sortColumn, ascending);
      }
    }
